...
stable_risk(Name) :-
  ask_marital_status(Name, Marital_Status), Marital_Status == married,
  ask_income(Name, Income), Income =< 50000,
  ask_mortgage(Name, Mortgage), Mortgage > 50000, !.
...
moderate_risk(Name) :-
  ask_marital_status(Name, Marital_Status), Marital_Status == single,
  ask_income(Name, Income), Income < 35000, !.

moderate_risk(Name) :-
  ask_marital_status(Name, Marital_Status), Marital_Status == single,
  ask_income(Name, Income), Income >= 35000,
  ask_age(Name, Age), Age > 50, !.
...
ask_marital_status(Name, Marital_Status) :-
  write('What is your marital status: married or single? '), read(Marital_Status),
  assert(marital_status(Name, Marital_Status)).

ask_income(Name, Income) :- income(Name, Income), !.
  ask_income(Name, Income) :- write('How much is your annual income? (RM) '), read(Income),
  assert(income(Name, Income)).

ask_mortgage(Name, Mortgage) :- mortgage(Name, Mortgage), !.
  ask_mortgage(Name, Mortgage) :- write('How much is your mortgage? (RM) '), read(Mortgage),
  assert(mortgage(Name, Mortgage)).

ask_age(Name, Age) :- age(Name, Age), !.
  ask_age(Name, Age) :- write('What is your age? '), read(Age),
  assert(age(Name, Age)).
